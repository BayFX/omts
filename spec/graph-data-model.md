# OMTS Specification: Graph Data Model
...
| Spec | Relationship |
|------|-------------|
| OMTS-SPEC-002 (Entity Identification) | Defines external identifier structures carried by nodes and edges defined here |
| OMTS-SPEC-003 (Merge Semantics) | Defines how graphs conforming to this model are combined |
| OMTS-SPEC-004 (Selective Disclosure) | Defines privacy controls for the `boundary_ref` node type and `person` node constraints |
| OMTS-SPEC-007 (Serialization Bindings) | Defines encoding formats (JSON, CBOR), compression, and serialization placement rules for the abstract model defined here |

---

## 1. Overview

This specification defines the OMTS supply chain graph data model: the file structure, node types, edge types, and structural validation rules that constitute a valid `.omts` file. It is the foundational specification upon which all other OMTS specs depend.
...
An `.omts` file encodes the graph data model defined in this section. The abstract model is encoding-independent; concrete serialization rules (JSON, CBOR) are defined in OMTS-SPEC-007. The top-level structure contains the following fields:

| Field | Required | Type | Description |
|-------|----------|------|-------------|
| `omts_version` | Yes | string | Semantic version of the OMTS specification (e.g., `"0.0.1"`) |
| `snapshot_date` | Yes | string (ISO 8601 date) | Date this graph snapshot was produced |
| `file_salt` | Yes | string | 64-character lowercase hexadecimal string (32 bytes). Generated by a CSPRNG. Used for boundary reference hashing (OMTS-SPEC-004). |
| `disclosure_scope` | No | enum | Intended audience: `internal`, `partner`, or `public`. See OMTS-SPEC-004 for enforcement rules. |
...
The `omts_version` field uses semantic versioning (`MAJOR.MINOR.PATCH`). The following rules define consumer behavior when encountering version mismatches:
...
**Pre-1.0 convention.** While `omts_version` has a major version of `0`, the format is considered unstable. Minor version increments MAY include breaking changes. Consumers SHOULD treat any version mismatch in the `0.x` series as potentially incompatible.
...
Edges MAY carry an optional `identifiers` array with the same structure as node identifiers (defined in OMTS-SPEC-002), enabling cross-file merge of edges when needed.

---

## 4. Node Types

OMTS defines six core node types and one structural type.
...
**Privacy constraints** for `person` nodes are defined in OMTS-SPEC-004.
...
A minimal node stub that replaces a redacted node in a subgraph projection, preserving graph connectivity without revealing the entity's identity. Defined in OMTS-SPEC-004.
...
**GLEIF Level 2 coverage limitation.** GLEIF Level 2 relationship data only covers entities where both parent and child hold LEIs — a minority of commercial entities globally. The absence of `legal_parentage` edges in an OMTS file does NOT mean an entity has no corporate parent; it means the relationship is not represented in this file. Consumers MUST NOT infer flat ownership structures from the absence of `legal_parentage` edges.
...
**Determining UBO status:** Under AMLD, a person is a UBO if they hold >25% of shares or voting rights, or exercise control through other means. OMTS records the raw ownership data; the 25% threshold determination is a tooling concern.

**Privacy:** `beneficial_ownership` edges inherit the sensitivity constraints of `person` nodes (see OMTS-SPEC-004). They default to `sensitivity: "confidential"` and MUST be omitted from files with `disclosure_scope: "public"`.
...
All nodes and edges MAY carry an optional `data_quality` object with the following fields. Serialization placement (top-level for nodes, inside the property wrapper for edges) is defined in OMTS-SPEC-007, Section 3.
...
**Serialization.** Labels follow the same placement convention as `data_quality` (Section 8.3): top-level on nodes, inside the property wrapper on edges. See OMTS-SPEC-007, Section 3 for concrete placement rules in each encoding.

**Key naming conventions:**

- Keys without a dot are reserved for future OMTS-defined vocabularies.
...
**Labels do NOT participate in identity predicates.** They are not considered during merge candidate detection (OMTS-SPEC-003, Section 2). During merge, labels are combined by set union of `{key, value}` pairs (OMTS-SPEC-003, Section 4).

> **Recommended vocabularies.** Appendix B defines recommended label keys for common supply chain, procurement, and regulatory classifications using OMTS-reserved (dotless) keys. Producers SHOULD prefer these standard keys over custom equivalents to maximize cross-organization interoperability.
...
**Cycle legality.** Cycles are permitted in the supply relationship subgraph (`supplies`, `subcontracts`, `tolls`, `distributes`, `brokers`, `sells_to`) — circular supply chains exist in practice (e.g., recycling loops). Cycles are NOT permitted in the `legal_parentage` subgraph (L3-MRG-02 in OMTS-SPEC-003 enforces this as a forest constraint). Cycles in `ownership` are permitted (cross-holdings are common).
...
A complete minimal `.omts` file demonstrating the graph data model in JSON serialization. For CBOR encoding rules, see OMTS-SPEC-007, Section 4.
...
1. Produce files that pass all L1 validation rules defined in this specification (Section 9.1), OMTS-SPEC-002 (Section 6.1), and OMTS-SPEC-004 (Section 6.1).
2. Set `omts_version` to a version of this specification that the producer implements.
3. Generate `file_salt` using a CSPRNG (Section 2).
4. Apply the edge property wrapper (Section 2.1) using the serialization rules defined in OMTS-SPEC-007.
...
7. Respect `disclosure_scope` sensitivity constraints (OMTS-SPEC-004, Section 3) when declared.
8. Support at least one serialization encoding defined in OMTS-SPEC-007.
...
1. Implement all L1 validation rules from this specification (Section 9.1), OMTS-SPEC-002 (Section 6.1), and OMTS-SPEC-004 (Section 6.1).
...
This appendix defines recommended label keys for common supply chain, procurement, and regulatory classifications. These keys use OMTS-reserved (dotless) names. Producers SHOULD prefer these keys over custom equivalents when the semantics match. Producers SHOULD use lowercase kebab-case for both keys and values.
