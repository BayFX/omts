...
| `previous_snapshot_ref` | No | string | SHA-256 hash (hex-encoded) or URI of the prior snapshot this file supersedes. Enables temporal audit trails for CSDDD/LkSG continuous monitoring. |
...
| `update_type` | No | enum | `snapshot` (default) or `delta`. See Section 12 for delta envelope semantics. |
...
> **Note on RFC 8259 key ordering.** JSON objects are formally unordered per RFC 8259 (Section 4: "An object is an unordered collection of zero or more name/value pairs"). The first-key requirement depends on serializer behavior — most serializers (Python `json`, JavaScript `JSON.stringify`, Java Jackson, Rust `serde_json`) preserve insertion order in practice, but this is not guaranteed by the JSON specification. Consumers SHOULD attempt first-key detection but MUST NOT reject files where `omtsf_version` is present but not the first key. When `omtsf_version` is not the first key, consumers SHOULD fall back to scanning all top-level keys.
...
| `risk_severity` | No | enum | Risk severity classification: `critical`, `high`, `medium`, `low`. Enables risk-prioritized due diligence under CSDDD Article 7. |
| `risk_likelihood` | No | enum | Likelihood of the identified risk: `very_likely`, `likely`, `possible`, `unlikely`. Combined with `risk_severity`, supports risk matrix prioritization. |
...
| `direct_emissions_co2e` | No | number | Direct (Scope 1) embedded emissions in tonnes CO2 equivalent. Required for CBAM reporting (Regulation (EU) 2023/956). |
| `indirect_emissions_co2e` | No | number | Indirect (Scope 2) embedded emissions in tonnes CO2 equivalent. CBAM default values may be used when actual data is unavailable. |
...
> **GLEIF Level 2 coverage note.** `legal_parentage` hierarchies derived from GLEIF Level 2 data may terminate at entities that do not hold LEIs. State-owned enterprises, sovereign wealth funds, family offices, and some holding companies are commonly absent from the LEI system. GLEIF Level 2 covers only the approximately 2.7 million LEI-holding entities and their direct accounting consolidation relationships. When building corporate hierarchies, producers SHOULD be aware that the topmost parent in a `legal_parentage` chain may not be the true ultimate parent but rather the highest entity in the chain that holds an LEI. Enrichment tooling MAY supplement GLEIF Level 2 data with national registry data (via `nat-reg` identifiers) to extend hierarchies beyond the LEI boundary.
...
> **Streaming and large files.** At the upper advisory size limits (1M nodes, 5M edges), an `.omts` file may reach hundreds of megabytes of JSON text. The `nodes` and `edges` top-level arrays are designed to support streaming/SAX-style parsing: a streaming JSON parser can process nodes and edges incrementally without loading the entire file into memory. Implementations processing large files SHOULD use a streaming parser (e.g., `ijson` in Python, `serde_json::StreamDeserializer` in Rust, Jackson `JsonParser` in Java) to avoid excessive memory allocation. A future specification version may define a compressed envelope format (e.g., `.omts.zst` using Zstandard compression) for efficient storage and transfer of large files.
...
1. Produce files that pass all L1 validation rules defined in this specification (Section 9.1), OMTSF-SPEC-002 (Section 6.1), and OMTSF-SPEC-004 (Section 8.1).
...
1. Implement all L1 validation rules from this specification (Section 9.1), OMTSF-SPEC-002 (Section 6.1), and OMTSF-SPEC-004 (Section 8.1).
...
Full-file re-export at enterprise scale (40,000+ vendors) is operationally expensive. This section defines a delta/patch mechanism that allows producers to transmit incremental changes to an existing snapshot.

### 12.1 Delta File Structure

A delta file is an `.omts` file with `update_type` set to `"delta"` in the file header. In addition to the standard header fields, a delta file MUST include:

| Field | Required | Type | Description |
|-------|----------|------|-------------|
| `update_type` | Yes | enum | MUST be `"delta"` |
| `base_snapshot_ref` | Yes | string | SHA-256 hash (hex-encoded) or URI of the base snapshot this delta applies to |
| `snapshot_date` | Yes | string (ISO 8601 date) | Date this delta was produced |

The `nodes` and `edges` arrays in a delta file contain **delta operations** rather than the full graph.

### 12.2 Delta Operations

Each node or edge object in a delta file carries an additional `_operation` field:

| Operation | Meaning |
|-----------|---------|
| `add` | The node or edge is new and MUST be added to the base graph. |
| `modify` | The node or edge exists in the base graph and MUST be updated with the provided properties. Properties not included in the delta are unchanged. |
| `remove` | The node or edge MUST be removed from the base graph. Only `id` and `type` are required; other fields MAY be omitted. |
...
### 12.3 Delta Application Rules

1. Consumers MUST apply delta operations in array order (nodes before edges).
2. An `add` operation for a node or edge whose `id` already exists in the base graph is an error.
3. A `modify` or `remove` operation for a node or edge whose `id` does not exist in the base graph is an error.
4. When an `add` or `modify` operation on an edge references a node `id` that does not exist in the base graph (after applying node deltas), the delta is invalid.
5. After applying all delta operations, the resulting graph MUST satisfy all L1 validation rules.

### 12.4 Disclosure Scope Inheritance

If the delta file does not declare a `disclosure_scope`, it inherits the `disclosure_scope` of the base snapshot. If the delta file declares a `disclosure_scope`, it MUST be equal to or more restrictive than the base snapshot's scope. The restriction ordering is: `public` < `partner` < `internal`.
