...
- **Namespace collision prevention.** Reserving dotless keys for future OMTSF vocabularies and recommending reverse-domain notation for custom keys mirrors the extension mechanism for node/edge types (Sections 8.1-8.2), creating a consistent namespacing pattern across the specification.
