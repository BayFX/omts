/// Subgraph extraction algorithms: induced subgraph and ego-graph.
///
...
/// # Output Validity
///
/// Both functions return a valid [`OmtsFile`] with the original header fields
/// preserved.  The `reporting_entity` header field is retained only if the
/// referenced node is present in the subgraph; otherwise it is set to `None`.
