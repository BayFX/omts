...
- **Complexity:** L
- **Crate:** `omtsf-excel` (template detection, import/export logic), `omtsf-cli` (format enum extension)
...
- **Acceptance Criteria:**
  ...
  - **Import auto-detection:** `omtsf import file.xlsx` inspects sheet names — if "Supplier List" sheet exists, uses the simplified parser; if "Organizations" sheet exists, uses the full multi-sheet parser; otherwise errors. No CLI flag change needed for import.
  - **Import logic** (`omtsf_excel::import_supplier_list`):
  ...
  - **Export logic** (`omtsf_excel::export_supplier_list`):
  ...
  - **README.md update:** The root `README.md` must be updated to:
    ...
    - Update the Repository Layout tree to show `templates/` instead of `tests/fixtures/` for Excel templates, and add `crates/omtsf-excel` to the `omtsf-rs/` tree
