# omts-rs Implementation Task Plan
...
- **Crate:** Both
- **Acceptance Criteria:**
  - Workspace `Cargo.toml` at `omts-rs/` with members `crates/omts-core` and `crates/omts-cli`
  - `omts-core` compiles as a library crate (`lib.rs`) with `serde`, `serde_json`, and `petgraph` dependencies declared
  - `omts-cli` compiles as a binary crate (`main.rs`) with `clap` dependency declared and a dependency on `omts-core`
  - Stub `omts-wasm` crate exists with an empty `lib.rs`
...
- **Acceptance Criteria:**
  ...
  - CI includes a `cargo check --target wasm32-unknown-unknown -p omts-core` step that verifies the core crate compiles to WASM
...
- **Acceptance Criteria:**
  ...
  - Fixtures are committed under `omts-rs/tests/fixtures/` and are reusable by all subsequent test tasks
...
- **Crate:** Both
- **Acceptance Criteria:**
  - Test: `omts init --example | omts validate -` exits 0
  - Test: `omts init --example | omts redact --scope public - | omts validate -` exits 0
  - Test: merge two fixture files, validate output, inspect output, diff output against expected
  - Test: `omts subgraph` output passes validation
...
- **Acceptance Criteria:**
  ...
  - Broken pipe handling verified: `omts inspect file.omts | head -1` exits 0, no error output
...
### T-047 -- Verify WASM compatibility of omts-core ✅
...
- **Crate:** omts-core
- **Acceptance Criteria:**
  - `cargo build --target wasm32-unknown-unknown -p omts-core` succeeds
  - No `std::fs`, `std::net`, `std::process` imports in `omts-core`
  - `sha2` and `getrandom` compile for wasm32 target with appropriate feature flags
  - Grep confirms no OS-level I/O anywhere in `omts-core` source
...
- **Crate:** Both
- **Acceptance Criteria:**
  - `omts-core` has a crate-level doc comment explaining purpose, WASM constraint, and module organization
  ...
  - `omts-cli/README.md` has build instructions and usage examples for all 10 commands
...
- **Acceptance Criteria:**
  ...
  - `omts_version` key present but position not enforced in CBOR
...
- **Acceptance Criteria:**
  ...
  - Fixtures committed under `omts-rs/tests/fixtures/` alongside existing JSON fixtures
...
- **Acceptance Criteria:**
  ...
  - `cargo build --target wasm32-unknown-unknown -p omts-core` succeeds (with or without compression feature)
  - Document any WASM limitations in `omts-rs/docs/data-model.md` Section 11
...
- **Acceptance Criteria:**
  ...
  - Since omts-core denies stdout/stderr but system time is fine, either get time in core or pass timestamp from CLI layer
...
- **Acceptance Criteria:**
  - Shared `#[cfg(test)]` test_helpers module in omts-core (or dev-dependency helper crate)
...
- **Complexity:** XL
- **Crate:** `omts-excel` (new crate; Excel I/O + tabular-to-graph conversion logic), `omts-cli` (CLI wiring only)
- **Description:** Create a new workspace crate `crates/omts-excel` that implements the Excel import logic. The crate depends on `omts-core` for graph types and `calamine` for `.xlsx` reading. The `omts-cli` crate depends on `omts-excel` and wires the `omts import` subcommand. The `--format <fmt>` flag defaults to `excel` for now but is designed to be extended with additional formats (e.g., CSV, Parquet) in the future. The template has 12 sheets: README, Metadata, Organizations, Facilities, Goods, Persons, Attestations, Consignments, Supply Relationships, Corporate Structure, Same As, and Identifiers.
- **Acceptance Criteria:**
  - A new crate `crates/omts-excel` is created as a workspace member with its own `Cargo.toml`; it depends on `omts-core` and `calamine`; it does NOT depend on `omts-cli`
  - `omts-excel` exposes a public `import_excel(reader: impl Read + Seek) -> Result<OmtsFile, ImportError>` function (or similar) that the CLI calls
  - `omts import [--format excel] <input.xlsx> [-o output.omts]` reads an Excel file and writes a valid `.omts` file
  ...
  - `calamine` dependency is confined to `omts-excel` (not in `omts-core` or `omts-cli`)
  ...
  - **Auto-generated fields:** `file_salt` via CSPRNG (always fresh), `omts_version` set to current, node/edge IDs generated as stable slugs when blank (e.g., `org-bolt-supplies-ltd`)
  ...
  - Output passes `omts validate` at L1 with zero errors
  - Integration test: `omts import --format excel tests/fixtures/excel/omts-import-example.xlsx | omts validate -` exits 0
...
- **Complexity:** L
- **Crate:** `omts-excel` (Excel I/O + graph-to-tabular conversion logic), `omts-cli` (CLI wiring only)
- **Description:** Add export functionality to the `omts-excel` crate (created in T-077). The export logic uses `rust_xlsxwriter` for `.xlsx` writing, added as a dependency of `omts-excel`. The `omts-cli` crate wires the `omts export` subcommand. The `--format` flag defaults to `excel` for now but is designed to be extended with additional formats in the future. This is the inverse of T-077 and enables round-trip workflows: import Excel → enrich/merge/redact → export back to Excel for review by procurement teams who don't use CLI tooling.
- **Acceptance Criteria:**
  - Export logic lives in `crates/omts-excel` alongside the import logic; `rust_xlsxwriter` dependency is confined to `omts-excel`
  - `omts-excel` exposes a public `export_excel(file: &OmtsFile, writer: impl Write) -> Result<(), ExportError>` function (or similar) that the CLI calls
  - `omts export [--format excel] <input.omts> -o output.xlsx` reads an `.omts` file and writes an Excel workbook
  - `--format` flag accepts `excel` (default); unknown formats produce a descriptive error and exit 2
  - The `ExportFormat` enum in clap is `#[non_exhaustive]` to signal future extensibility
  - `rust_xlsxwriter` dependency is confined to `omts-excel` (not in `omts-core` or `omts-cli`)
  - **Metadata sheet:** populated from file header fields (`omts_version`, `snapshot_date`, `reporting_entity`, `disclosure_scope`)
  ...
  - Round-trip test: `omts import --format excel example.xlsx | omts export --format excel - -o roundtrip.xlsx` → re-import roundtrip.xlsx → validate → diff against original import shows zero structural differences (node/edge counts, identifiers, edge properties match)
...
- **Complexity:** L
- **Crate:** `omts-excel` (template detection, import/export logic), `omts-cli` (format enum extension)
- **Description:** Design and implement a simplified single-sheet Excel template ("Supplier List") that auto-generates organization nodes and `supplies` edges from a flat table. This is the P1 panel recommendation for adoption: procurement teams will not fill 12 sheets for a simple tier-1 supplier list. The template uses a single "Supplier List" sheet with inline metadata (rows 1–2), column headers (row 4), and data rows (row 5+). Each row represents one supplier; the import logic auto-generates an org node per unique supplier name and `supplies` edges linking each supplier to the reporting entity (tier 1) or to a named parent supplier (tier 2/3). The CLI format enums (`ImportFormat`, `ExportFormat`) are extended: import auto-detects the template variant by inspecting sheet names; export requires explicit selection (`--output-format excel` for full, `--output-format excel-supplier-list` for simplified). The architecture must support additional template variants in the future without breaking changes.
- **Acceptance Criteria:**
  - **Template files** exist at `templates/excel/omts-supplier-list-template.xlsx` (empty) and `omts-supplier-list-example.xlsx` (populated with example data) — generated by `generate_template.py`
  - **Sheet layout:** Single sheet named "Supplier List" with:
    - Rows 1–2: Metadata key-value pairs (`Reporting Entity`, `Snapshot Date`, `Disclosure Scope`) in a styled header area
    - Row 3: Blank separator
    - Row 4: Column headers with data validation dropdowns on enum columns
    - Row 5+: Data rows
  - **Columns:** `supplier_name` (required), `jurisdiction`, `tier` (1/2/3, default 1), `parent_supplier` (name-based ref for tier 2/3), `commodity`, `valid_from`, `annual_value`, `value_currency`, `contract_ref`, `lei`, `duns`, `vat`, `vat_country`, `internal_id`, `risk_tier` (label), `kraljic_quadrant` (label), `approval_status` (label), `notes` (free text, preserved in `extra` but not part of the graph model)
  - **Import auto-detection:** `omts import file.xlsx` inspects sheet names — if "Supplier List" sheet exists, uses the simplified parser; if "Organizations" sheet exists, uses the full multi-sheet parser; otherwise errors. No CLI flag change needed for import.
  - **Import logic** (`omts_excel::import_supplier_list`):
    - Reads metadata from rows 1–2 (reporting entity name, snapshot date, disclosure scope); uses sensible defaults when blank (snapshot_date = today, disclosure_scope = "partner")
    - Auto-generates a reporting entity organization node from the metadata `Reporting Entity` value
    - Creates one organization node per unique `supplier_name` value; de-duplicates by name (rows with the same supplier_name are the same org)
    - Creates `supplies` edges: tier-1 suppliers → reporting entity; tier-2 → their `parent_supplier`; tier-3 → their `parent_supplier` (which must be a tier-2 name in the same file)
    - Populates identifiers (lei, duns, vat+country, internal_id) on org nodes
    - Maps `risk_tier`, `kraljic_quadrant`, `approval_status` to OMTS labels on org nodes
    - Maps `commodity`, `valid_from`, `annual_value`, `value_currency`, `contract_ref` to edge properties on the `supplies` edge
    - Auto-generates node IDs from slugified supplier names, edge IDs from slugified source+target
    - Auto-generates `file_salt` (CSPRNG)
    - Validates: `parent_supplier` references must resolve to a supplier_name in the file; tier-2/3 rows must have a `parent_supplier`; at least one `supplier_name` must be non-empty
    - Runs L1 validation on the constructed graph; blocks output on L1 errors
  - **Export logic** (`omts_excel::export_supplier_list`):
    - Writes a single "Supplier List" sheet from an `OmtsFile`
    - Extracts organizations + supplies edges; computes tier from graph distance to reporting entity
    - Resolves `parent_supplier` from the `supplies` edge source chain
    - Writes metadata rows, then data rows sorted by tier then name
    - Only works cleanly on supplier-list-shaped graphs; non-organization nodes and non-supplies edges are silently omitted (or produce a warning)
  - **CLI changes:**
    - `ExportFormat` enum gains `ExcelSupplierList` variant (displayed as `excel-supplier-list` in clap); `ExportFormat::Excel` remains the default
    - `ImportFormat` stays as `Excel` (auto-detection is internal)
    - `--output-format excel-supplier-list` triggers `export_supplier_list`
  - **Architecture for future templates:**
    - Internal `ExcelVariant` enum (or similar) with `Full` and `SupplierList` variants, extensible with `#[non_exhaustive]`
    - `detect_excel_variant(workbook) -> Result<ExcelVariant, ImportError>` function that inspects sheet names
    - Import dispatches to the appropriate parser based on detected variant
    - Export dispatches based on `ExportFormat` enum variant
  - **Round-trip test:** import `omts-supplier-list-example.xlsx` → export with `--output-format excel-supplier-list` → re-import → validate → node/edge counts match
  - **Integration tests:**
    - `import_supplier_list_example_exits_0`: import the example fixture, validate output
    - `import_supplier_list_template_exits_0`: import the empty template (produces minimal valid output)
    - `import_supplier_list_pipe_to_validate_exits_0`: import → pipe to validate → exit 0
    - `import_supplier_list_node_counts`: verify auto-generated org count and edge count (example has 7 orgs including reporting entity, 6 supplies edges)
    - `import_supplier_list_tier_hierarchy`: verify tier-2 and tier-3 edges point to correct parents
    - `import_supplier_list_labels`: verify risk_tier/kraljic_quadrant/approval_status become OMTS labels
    - `import_supplier_list_bad_parent_ref_exits_1`: tier-2 row with unknown `parent_supplier` produces error
    - `export_supplier_list_exits_0`: export a fixture to supplier-list format
    - `export_supplier_list_round_trip`: full round-trip as described above
    - `export_unknown_format_exits_2`: `--output-format foobar` produces exit 2
  - Full multi-sheet import/export continues to work unchanged (no regressions)
  - **Directory restructure:** Move `tests/fixtures/excel/` to `templates/excel/` at the repo root. Excel templates are user-facing artifacts (not test-only fixtures), so they belong under a dedicated top-level `templates/` directory. All references in code, tests, docs, CI, and `generate_template.py` must be updated to use the new path. The `.omts` JSON fixtures remain at `tests/fixtures/` (they are test-only).
  - **README.md update:** The root `README.md` must be updated to:
    - Add `import` and `export` commands to the CLI Commands table (added by T-077/T-078 but not yet reflected in the README)
    - Add an **Excel Support** section describing both template variants:
      - **Rich template** (multi-sheet): full-fidelity round-trip for all node/edge types, identifiers, attestations — suited for data engineers and tooling integration
      - **Simplified template** ("Supplier List", single-sheet): flat tier-1/2/3 supplier table with auto-generated graph structure — suited for procurement teams filling out supplier data manually
    - Mention auto-detection on import (sheet-name-based) and explicit `--output-format` selection on export
    - Update the Repository Layout tree to show `templates/` instead of `tests/fixtures/` for Excel templates, and add `crates/omts-excel` to the `omts-rs/` tree
