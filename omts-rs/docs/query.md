# omts-core Technical Specification: Query & Subchain Extraction
...
The selector types and query functions are placed in `omts-core::graph::extraction`, alongside the existing `induced_subgraph` and `ego_graph` functions.
...
### 5.2 `omts subgraph` with Selector Flags
...
This section specifies benchmarks for the selector query engine, following the patterns established in the existing benchmark suite (see `omts-bench/benches/subgraph_extraction.rs` and `omts-bench/benches/graph_queries.rs`).

### 6.1 Benchmark File

**New file:** `omts-bench/benches/selector_query.rs`
...
The existing generator (`omts-bench/src/generator/`) must produce fixtures with sufficient label and identifier coverage for meaningful selector benchmarks.
...
```
omts subgraph graph.omts --jurisdiction DE | omts validate -
omts subgraph graph.omts --node-type organization | omts inspect -
omts subgraph a.omts --label certified | omts diff - <(omts subgraph b.omts --label certified)
```
