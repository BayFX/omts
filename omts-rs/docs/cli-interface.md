# omts-cli Technical Specification: CLI Interface
...
This document specifies the complete command-line interface for `omts-cli`: argument structure, flags, file I/O behavior, encoding detection, output formatting, and exit codes. It is the authoritative reference for the binary surface area. The library API (`omts-core`) is specified in separate documents; this document covers only the CLI layer.

All commands follow the form `omts <subcommand> [options] [arguments]`.
...
| `--max-file-size <bytes>` | | u64 | 268435456 (256 MB) | Maximum file size in bytes. Also settable via `OMTS_MAX_FILE_SIZE` env var. CLI flag takes precedence over env var. |
...
| `--version` | `-V` | | | Print `omts <version>` and exit. |

### clap Derive Structure

```rust
#[derive(Parser)]
#[command(name = "omts", version, about = "OMTS reference CLI")]
struct Cli {
    #[command(subcommand)]
    command: Command,
    ...
    #[arg(long, global = true, env = "OMTS_MAX_FILE_SIZE",
...
### 3.1 `omts validate <file>`

Validates a single `.omts` file against the OMTS specification suite.
...
**Examples:**
```
omts validate supply-chain.omts
omts validate --level 3 supply-chain.omts
cat supply-chain.omts | omts validate -
omts validate -f json supply-chain.omts 2> findings.ndjson
omts validate supply-chain.omts.zst
```
...
**Examples:**
```
omts merge file-a.omts file-b.omts > merged.omts
omts merge --strategy intersect a.omts b.omts c.omts > common.omts
omts merge --to cbor --compress a.omts b.omts > merged.omts.zst
cat remote.omts | omts merge - local.omts > combined.omts
```

### 3.3 `omts redact <file>`
...
**Examples:**
```
omts redact --scope public supply-chain.omts > public.omts
omts redact --scope partner internal.omts | omts validate -
omts redact --scope public --to cbor --compress data.omts > public.omts.zst
```

### 3.4 `omts inspect <file>`
...
**Examples:**
```
omts inspect supply-chain.omts
omts inspect -f json supply-chain.omts | jq .node_counts
omts inspect data.omts.zst
```
...
**Examples:**
```
omts diff v1.omts v2.omts
omts diff --ids-only baseline.omts current.omts
omts diff -f json old.omts new.omts > changes.json
omts diff json-version.omts cbor-version.omts
```

### 3.6 `omts convert <file>`
...
**Examples:**
```
omts convert messy.omts > clean.omts
omts convert --compact supply-chain.omts | wc -c
omts convert --to cbor supply-chain.omts > supply-chain.cbor.omts
omts convert --to cbor --compress data.omts > data.omts.zst
omts convert --to json data.cbor.omts > data.omts
cat untrusted.omts | omts convert - > normalized.omts
```

### 3.7 `omts reach <file> <node-id>`
...
**Examples:**
```
omts reach supply-chain.omts org-001
omts reach --depth 3 supply-chain.omts org-001
omts reach --direction both -f json graph.omts node-42
```

### 3.8 `omts path <file> <from> <to>`
...
**Examples:**
```
omts path supply-chain.omts org-001 facility-099
omts path --max-paths 3 graph.omts src dst
```

### 3.9 `omts subgraph <file> [node-id...] [selectors]`
...
**Selector flags:** (all repeatable, same as `omts query` Section 3.11)
...
**Examples:**
```
omts subgraph supply-chain.omts org-001 org-002 > pair.omts
omts subgraph --expand 2 graph.omts org-001 > neighborhood.omts
omts subgraph --to cbor --compress graph.omts org-001 org-002 > pair.omts.zst
omts subgraph supply-chain.omts --node-type organization --jurisdiction DE > german-orgs.omts
omts subgraph supply-chain.omts --identifier lei --expand 2 > lei-neighborhood.omts
omts subgraph graph.omts --label tier=1 --expand 0 > tier1-only.omts
omts subgraph graph.omts org-001 --node-type facility --expand 1 > combined.omts
cat graph.omts | omts subgraph - --name "Acme" > acme-subgraph.omts
```
...
**Examples:**
```
omts init > new-graph.omts
omts init --example > demo.omts
omts init --example | omts validate -
omts init --to cbor > new-graph.cbor.omts
```

### 3.11 `omts query <file> [selectors]`
...
omts query supply-chain.omts --label certified --name "Acme"
omts query -f json graph.omts --identifier lei --count
omts query graph.omts --edge-type supplies --label tier=1
```
...
File I/O is exclusively the CLI's concern. `omts-core` never touches the filesystem; it operates on `&str` and `&[u8]` inputs.
...
When `-` is provided, the CLI reads the entire stdin stream into a byte buffer before passing it to the parser. This is necessary because `omts-core` operates on `&str` / `&[u8]` inputs, not streaming readers. Stdin is not seekable, so the full contents must be buffered.
...
After detection, the byte buffer is passed to the appropriate parser in `omts-core`:
...
#[derive(Subcommand)]
enum Command {
    /// Validate an .omts file against the OMTS specification.
    Validate {
        #[arg(value_name = "FILE")]
        file: PathOrStdin,
...
|----------|---------|---------------|
| `OMTS_MAX_FILE_SIZE` | Default file size limit in bytes | `--max-file-size` flag |
