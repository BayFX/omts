# omts-core Technical Specification: Data Model
...
This document specifies the Rust type definitions in `omts-core` that represent the OMTS graph data model. It covers the complete type hierarchy for files, nodes, edges, identifiers, and labels; the serde strategy for JSON round-trip fidelity; the modeling decisions for graph-local references; and WASM compatibility constraints.

All section references of the form `SPEC-001 Section N` refer to the OMTS Graph Data Model specification. References of the form `SPEC-002 Section N` refer to the Entity Identification specification.
...
pub struct OmtsFile {
    ...
    /// All JSON fields not recognized by this version of omts-core.
    #[serde(flatten)]
...
All structs use `#[serde(rename_all = "snake_case")]`. This is the default JSON key style in OMTS (e.g., `omts_version`, `snapshot_date`, `file_salt`). The `type` field on nodes and edges uses `#[serde(rename = "type")]` since `type` is a Rust keyword.
...
The spec states that `omts_version` MUST be the first key in the top-level JSON object (SPEC-001 Section 2.1). `serde_json` serializes struct fields in declaration order, so the field order in `OmtsFile` is load-bearing. The `extra` flattened map is emitted last.
...
No other dependencies are permitted in the data model module. The `omts-core` crate enforces this boundary with `#![deny(clippy::print_stdout, clippy::print_stderr)]` at the crate level.
...
```
cargo build -p omts-core --target wasm32-unknown-unknown --no-default-features
```
...
The `OmtsFile` struct and its children are not directly `#[wasm_bindgen]`-annotated. wasm-bindgen cannot handle complex nested Rust types. Instead, the future `omts-wasm` crate will expose a thin JS-facing API that accepts JSON strings, deserializes into these types internally, and returns results as JSON or simple scalar values. The types defined here are the internal representation, not the FFI boundary.

### 11.4 Serialization to/from JS

For WASM consumers that need the full parsed tree on the JS side, `serde-wasm-bindgen` can convert `OmtsFile` to a `JsValue` without an intermediate JSON string. This is an `omts-wasm` crate concern and does not affect the type definitions here.

---

## 12. Module Layout

The data model types are split across four source modules in `omts-core`:
