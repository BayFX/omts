# omts-core Technical Specification: Merge Engine
...
This document specifies the merge engine in `omts-core`, implementing SPEC-003: given two or more `.omts` files describing overlapping supply chain subgraphs, produce a single deduplicated graph. The engine guarantees commutativity, associativity, and idempotency (SPEC-003 S5) so that independent parties merging overlapping files in any order converge on an identical result.
...
Hand-crafted fixture pairs in `omts-cli/tests/` cover: disjoint graphs, full overlap, conflicting properties, transitive chains (A-B via LEI, B-C via DUNS), `same_as` at each confidence level, temporal incompatibility, ANNULLED LEI exclusion, oversized groups, and colliding file-local node IDs.
