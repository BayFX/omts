# omts-cli Technical Specification: Overview
...
`omts-cli` is the reference command-line tool for working with `.omts` files. It validates, merges, redacts, inspects, diffs, converts, queries, and scaffolds supply chain graph files as defined by the OMTS specification suite (SPEC-001 through SPEC-006).

The CLI is the primary interface for integrating OMTS into automated pipelines, CI/CD workflows, and developer toolchains. It is designed for implementors building on or contributing to the OMTS ecosystem.
...
├── crates/
│   ├── omts-core/         # library: data model, parsing, validation, merge, graph engine
│   │   ├── Cargo.toml
│   │   └── src/
│   ├── omts-cli/          # binary: argument parsing, I/O, formatting, exit codes
│   │   ├── Cargo.toml
│   │   └── src/
│   └── omts-wasm/         # future: wasm-bindgen surface (not specced yet)
│       └── Cargo.toml
├── docs/                   # technical specification documents
└── tests/                  # integration tests, fixture .omts files
```

**`omts-core`** is the library. It owns:
...
`omts-core` has **no** dependency on `std::fs`, `std::io::Read` from files, or any OS-level I/O. It operates on `&str`, `&[u8]`, or `serde_json::Value` inputs. This is the WASM boundary constraint.

**`omts-cli`** is the binary. It owns:
...
**`omts-wasm`** is a future crate. The current spec ensures `omts-core` is compatible with wasm32 targets but does not define the wasm-bindgen surface.
...
|---------|-------|--------|----------------|
| `omts validate <file>` | One `.omts` file | Diagnostics (errors, warnings, info) | SPEC-001 §9, SPEC-002 §6, SPEC-004 §6 |
| `omts merge <file>...` | Two or more `.omts` files | Merged `.omts` to stdout | SPEC-003 |
| `omts redact <file>` | One `.omts` file + scope flag | Redacted `.omts` to stdout | SPEC-004 |
| `omts inspect <file>` | One `.omts` file | Summary statistics to stdout | — |
| `omts diff <a> <b>` | Two `.omts` files | Structural diff to stdout | — |
| `omts convert <file>` | One `.omts` file | Re-serialized `.omts` to stdout | — |

### 4.2 Graph Query Commands

| Command | Input | Output | Description |
|---------|-------|--------|-------------|
| `omts reach <file> <node-id>` | File + source node | Reachable node set | All nodes reachable from the given node via directed edges |
| `omts path <file> <from> <to>` | File + two node IDs | Paths | All or shortest paths between two nodes |
| `omts subgraph <file> [node-id...] [selectors]` | File + node IDs and/or selector flags | `.omts` subgraph to stdout | Extract the induced subgraph by explicit node IDs, property-based selectors, or both |
| `omts query <file> [selectors]` | File + selector flags | Matching nodes/edges to stdout | Display nodes and edges matching property-based selectors (type, label, identifier, jurisdiction, name) |

### 4.3 Scaffolding Commands

| Command | Input | Output | Description |
|---------|-------|--------|-------------|
| `omts init` | None | Minimal `.omts` to stdout | Minimal valid file: header with generated `file_salt`, empty `nodes`/`edges` arrays |
| `omts init --example` | None | Example `.omts` to stdout | Realistic sample file for learning and testing |
...
|-------|---------|---------|
| `serde`, `serde_json` | JSON serialization/deserialization of `.omts` files | `omts-core` |
| `clap` | CLI argument parsing, subcommand dispatch, help generation | `omts-cli` |
| `petgraph` | Directed graph construction, traversal, path queries | `omts-core` |
...
- **Override:** `--max-file-size <bytes>` flag or `OMTS_MAX_FILE_SIZE` environment variable
- **Behavior on exceed:** reject with a clear error message before parsing begins

This is a CLI concern, not a library concern. `omts-core` accepts parsed data structures and does not enforce file-level size limits.
