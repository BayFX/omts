# omts-core Technical Specification: Diff Engine
...
This document specifies the structural diff engine in `omts-core`. The engine compares two parsed `.omts` files and produces a typed description of what changed: which nodes and edges were added, removed, or modified, and which properties within matched elements differ.
...
The engine lives in `omts-core` and operates on deserialized `&OmtsFile` values. It has no filesystem dependency and is WASM-compatible. The CLI's `omts diff <a> <b>` command handles file I/O, calls the library, and formats the output.
...
/// The complete result of a structural diff between two OMTS files.
...
```rust
/// Compares two parsed OMTS files and returns a full diff.
/// File A is the baseline ("before"); file B is the target ("after").
pub fn diff(a: &OmtsFile, b: &OmtsFile) -> DiffResult;

/// Compares two parsed OMTS files with an optional filter.
...
The `omts diff <a> <b>` command reads two `.omts` files, parses both into `OmtsFile` values, calls `diff` (or `diff_filtered` if filters are provided), and writes the result to stdout.
...
**Version mismatch.** If the two files declare different `omts_version` values, the engine emits a warning but proceeds. Property comparison still works -- unknown fields in either file are captured as `Value` and compared structurally.
