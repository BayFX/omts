//! Integration tests for `omts path`.
...
fn path_max_paths_limits_output() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts path --max-paths 1");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn path_max_depth_too_short_exits_1() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts path --max-depth 2");
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn path_no_path_exits_1() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts path with no route");
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn path_unknown_from_node_exits_1() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts path with unknown from node");
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn path_unknown_to_node_exits_1() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts path with unknown to node");
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn path_nonexistent_file_exits_2() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts path nonexistent");
    assert_eq!(
        out.status.code(),
        Some(2),
