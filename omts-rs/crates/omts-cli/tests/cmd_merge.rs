//! Integration tests for `omts merge`.
...
fn merge_output_is_valid_omts_file() {
    ...
    assert!(
        value["omts_version"].is_string(),
        "omts_version must be present"
    );
    assert!(
...
fn merge_invalid_json_file_exits_2() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts merge bad json");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn merge_nonexistent_file_exits_2() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts merge nonexistent");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn merge_one_file_is_clap_error() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts merge one file");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn merge_stdin_and_file_exits_0() {
    ...
    let mut child = Command::new(omts_bin())
        .args([
            "merge",
            "-",
            fixture("merge-a.omts").to_str().expect("path"),
...
fn merge_output_passes_validate() {
    ...
    let validate_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate on merged output");
    assert_eq!(
...
/// `--to cbor` output is accepted by `omts validate`.
#[test]
fn merge_to_cbor_passes_validate() {
    ...
    let validate_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate on merged CBOR output");
    assert_eq!(
        validate_out.status.code(),
        Some(0),
...
fn merge_compress_starts_with_zstd_magic() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts merge --compress");
    assert_eq!(
        out.status.code(),
        Some(0),
        "expected exit 0; stderr: {}",
...
fn merge_cbor_compress_passes_validate() {
    let merge_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts merge --to cbor --compress");
    assert_eq!(
    ...
    let validate_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate on merged cbor+zstd output");
    assert_eq!(
        validate_out.status.code(),
        Some(0),
