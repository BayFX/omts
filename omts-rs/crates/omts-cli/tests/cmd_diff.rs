//! Integration tests for `omts diff`.
...
fn diff_ids_only_identical_files_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts diff --ids-only identical");
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn diff_summary_only_identical_files_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts diff --summary-only identical");
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn diff_added_node_shows_plus_prefix() {
  ...
  "edges": []
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts diff (added node)");
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("  + "),
...
fn diff_node_type_filter_unchanged_type_exits_0() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts diff --node-type facility");
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn diff_ignore_field_name_exits_0() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts diff --ignore-field name");
    assert_eq!(
        out.status.code(),
        Some(0),
