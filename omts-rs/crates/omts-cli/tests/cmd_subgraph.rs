//! Integration tests for `omts subgraph`.
#![allow(clippy::expect_used)]

use std::path::PathBuf;
use std::process::Command;

/// Path to the compiled `omts` binary.
fn omts_bin() -> PathBuf {
    let mut path = std::env::current_exe().expect("current exe");
    path.pop();
    if path.ends_with("deps") {
        path.pop();
    }
    path.push("omts");
    path
}

/// Path to a shared fixture file.
fn fixture(name: &str) -> PathBuf {
    let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    path.push("../../tests/fixtures");
    path.push(name);
    path
}

#[test]
fn subgraph_single_node_exits_0() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
        ])
        .output()
        .expect("run omts subgraph");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
}

#[test]
fn subgraph_output_is_valid_json() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
        ])
        .output()
        .expect("run omts subgraph");
    let stdout = String::from_utf8_lossy(&out.stdout);
    let parsed: Result<serde_json::Value, _> = serde_json::from_str(stdout.trim());
    assert!(parsed.is_ok(), "output is not valid JSON: {stdout}");
}

#[test]
fn subgraph_output_is_valid_omts_file() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "org-b",
        ])
        .output()
        .expect("run omts subgraph");
    let stdout = String::from_utf8_lossy(&out.stdout);
    let value: serde_json::Value =
        serde_json::from_str(stdout.trim()).expect("valid JSON from subgraph");
    assert!(value.get("omts_version").is_some(), "missing omts_version");
    assert!(
        value.get("snapshot_date").is_some(),
        "missing snapshot_date"
    );
    assert!(value.get("file_salt").is_some(), "missing file_salt");
    assert!(value.get("nodes").is_some(), "missing nodes");
    assert!(value.get("edges").is_some(), "missing edges");
}

#[test]
fn subgraph_two_nodes_includes_edge_between_them() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "org-b",
        ])
        .output()
        .expect("run omts subgraph org-a org-b");
    let stdout = String::from_utf8_lossy(&out.stdout);
    let value: serde_json::Value =
        serde_json::from_str(stdout.trim()).expect("valid JSON from subgraph");

    let nodes = value["nodes"].as_array().expect("nodes array");
    assert_eq!(nodes.len(), 2, "expected exactly 2 nodes");

    let edges = value["edges"].as_array().expect("edges array");
    assert_eq!(edges.len(), 1, "expected exactly 1 edge (e-ab)");

    let node_ids: Vec<&str> = nodes.iter().filter_map(|n| n["id"].as_str()).collect();
    assert!(node_ids.contains(&"org-a"), "org-a should be in subgraph");
    assert!(node_ids.contains(&"org-b"), "org-b should be in subgraph");
}

#[test]
fn subgraph_two_non_adjacent_nodes_has_no_edges() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "org-d",
        ])
        .output()
        .expect("run omts subgraph org-a org-d");
    let stdout = String::from_utf8_lossy(&out.stdout);
    let value: serde_json::Value =
        serde_json::from_str(stdout.trim()).expect("valid JSON from subgraph");

    let edges = value["edges"].as_array().expect("edges array");
    assert!(
        edges.is_empty(),
        "no edge between org-a and org-d: {stdout}"
    );
}

#[test]
fn subgraph_expand_1_includes_neighbours() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            "--expand",
            "1",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-b",
        ])
        .output()
        .expect("run omts subgraph --expand 1 org-b");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    let stdout = String::from_utf8_lossy(&out.stdout);
    let value: serde_json::Value =
        serde_json::from_str(stdout.trim()).expect("valid JSON from subgraph");

    let nodes = value["nodes"].as_array().expect("nodes array");
    let node_ids: Vec<&str> = nodes.iter().filter_map(|n| n["id"].as_str()).collect();

    assert!(node_ids.contains(&"org-b"), "org-b must be present");
    assert!(
        node_ids.contains(&"org-a"),
        "org-a is 1 hop incoming from org-b"
    );
    assert!(
        node_ids.contains(&"org-c"),
        "org-c is 1 hop outgoing from org-b"
    );
}

#[test]
fn subgraph_expand_0_is_same_as_no_expand() {
    let out_default = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "org-b",
        ])
        .output()
        .expect("run omts subgraph (no expand)");

    let out_explicit = Command::new(omts_bin())
        .args([
            "subgraph",
            "--expand",
            "0",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "org-b",
        ])
        .output()
        .expect("run omts subgraph --expand 0");

    assert_eq!(
        out_default.stdout, out_explicit.stdout,
        "expand 0 == default"
    );
}

#[test]
fn subgraph_unknown_node_exits_1() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
            "no-such-node",
        ])
        .output()
        .expect("run omts subgraph with unknown node");
    assert_eq!(
        out.status.code(),
        Some(1),
        "expected exit 1 for unknown node"
    );
}

#[test]
fn subgraph_nonexistent_file_exits_2() {
    let out = Command::new(omts_bin())
        .args(["subgraph", "/no/such/file/ever.omts", "org-a"])
        .output()
        .expect("run omts subgraph nonexistent");
    assert_eq!(
        out.status.code(),
        Some(2),
        "expected exit 2 for nonexistent file"
    );
}

#[test]
fn subgraph_invalid_json_exits_2() {
    use std::io::Write as _;
    let mut tmp = tempfile::NamedTempFile::new().expect("temp file");
    tmp.write_all(b"not-json").expect("write");
    let out = Command::new(omts_bin())
        .args(["subgraph", tmp.path().to_str().expect("path"), "org-a"])
        .output()
        .expect("run omts subgraph bad-json");
    assert_eq!(
        out.status.code(),
        Some(2),
        "expected exit 2 for invalid JSON"
    );
}

#[test]
fn subgraph_to_cbor_exits_0() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            "--to",
            "cbor",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "org-b",
        ])
        .output()
        .expect("run omts subgraph --to cbor");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
}

#[test]
fn subgraph_to_cbor_starts_with_cbor_tag() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            "--to",
            "cbor",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "org-b",
        ])
        .output()
        .expect("run omts subgraph --to cbor");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    assert!(
        out.stdout.len() >= 3,
        "CBOR output should have at least 3 bytes"
    );
    assert_eq!(
        &out.stdout[..3],
        &[0xD9, 0xD9, 0xF7],
        "CBOR output must start with self-describing tag 55799"
    );
}

#[test]
fn subgraph_compress_exits_0() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            "--compress",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "org-b",
        ])
        .output()
        .expect("run omts subgraph --compress");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
}

#[test]
fn subgraph_compress_starts_with_zstd_magic() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            "--compress",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "org-b",
        ])
        .output()
        .expect("run omts subgraph --compress");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    assert!(
        out.stdout.len() >= 4,
        "compressed output should have at least 4 bytes"
    );
    assert_eq!(
        &out.stdout[..4],
        &[0x28, 0xB5, 0x2F, 0xFD],
        "compressed output must start with zstd magic bytes"
    );
}

#[test]
fn subgraph_to_cbor_compress_starts_with_zstd_magic() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            "--to",
            "cbor",
            "--compress",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "org-b",
        ])
        .output()
        .expect("run omts subgraph --to cbor --compress");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    assert!(
        out.stdout.len() >= 4,
        "compressed output should have at least 4 bytes"
    );
    assert_eq!(
        &out.stdout[..4],
        &[0x28, 0xB5, 0x2F, 0xFD],
        "compressed CBOR output must start with zstd magic bytes"
    );
}

#[test]
fn subgraph_output_passes_validate() {
    // Write the subgraph JSON to a temp file and pipe it through `omts validate`.
    use std::io::Write as _;

    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "org-b",
        ])
        .output()
        .expect("run omts subgraph");
    assert!(
        out.status.success(),
        "subgraph exit code: {:?}",
        out.status.code()
    );

    let mut tmp = tempfile::NamedTempFile::new().expect("temp file");
    tmp.write_all(&out.stdout).expect("write subgraph output");

    let validate_out = Command::new(omts_bin())
        .args(["validate", tmp.path().to_str().expect("path")])
        .output()
        .expect("run omts validate on subgraph output");
    assert!(
        validate_out.status.success(),
        "subgraph output should pass validation, exit code: {:?}\nstderr: {}",
        validate_out.status.code(),
        String::from_utf8_lossy(&validate_out.stderr)
    );
}

#[test]
fn subgraph_selector_node_type_exits_0() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
            "--node-type",
            "organization",
        ])
        .output()
        .expect("run omts subgraph --node-type organization");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    let stdout = String::from_utf8_lossy(&out.stdout);
    let value: serde_json::Value =
        serde_json::from_str(stdout.trim()).expect("valid JSON from subgraph");
    let nodes = value["nodes"].as_array().expect("nodes array");
    assert!(
        !nodes.is_empty(),
        "should have at least one organization node"
    );
}

#[test]
fn subgraph_selector_with_expand() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("full-featured.omts").to_str().expect("path"),
            "--node-type",
            "facility",
            "--expand",
            "1",
        ])
        .output()
        .expect("run omts subgraph --node-type facility --expand 1");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    let stdout = String::from_utf8_lossy(&out.stdout);
    let value: serde_json::Value =
        serde_json::from_str(stdout.trim()).expect("valid JSON from subgraph");
    let nodes = value["nodes"].as_array().expect("nodes array");
    assert!(
        nodes.len() > 1,
        "expand=1 from facility should include neighbors"
    );
}

#[test]
fn subgraph_combined_node_ids_and_selectors() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
            "org-a",
            "--node-type",
            "organization",
        ])
        .output()
        .expect("run omts subgraph org-a --node-type organization");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    let stdout = String::from_utf8_lossy(&out.stdout);
    let value: serde_json::Value =
        serde_json::from_str(stdout.trim()).expect("valid JSON from subgraph");
    let nodes = value["nodes"].as_array().expect("nodes array");
    let node_ids: Vec<&str> = nodes.iter().filter_map(|n| n["id"].as_str()).collect();
    assert!(
        node_ids.contains(&"org-a"),
        "explicit node ID org-a must be present"
    );
}

#[test]
fn subgraph_no_ids_no_selectors_exits_2() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
        ])
        .output()
        .expect("run omts subgraph with no args");
    assert_eq!(
        out.status.code(),
        Some(2),
        "expected exit 2 when no node IDs and no selectors"
    );
}

#[test]
fn subgraph_selector_to_cbor_exits_0() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            "--to",
            "cbor",
            fixture("graph-query.omts").to_str().expect("path"),
            "--node-type",
            "organization",
        ])
        .output()
        .expect("run omts subgraph --node-type org --to cbor");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    assert!(
        out.stdout.len() >= 3,
        "CBOR output should have at least 3 bytes"
    );
    assert_eq!(
        &out.stdout[..3],
        &[0xD9, 0xD9, 0xF7],
        "CBOR output must start with self-describing tag 55799"
    );
}

#[test]
fn subgraph_selector_compress_exits_0() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            "--compress",
            fixture("graph-query.omts").to_str().expect("path"),
            "--node-type",
            "organization",
        ])
        .output()
        .expect("run omts subgraph --node-type org --compress");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    assert!(
        out.stdout.len() >= 4,
        "compressed output should have at least 4 bytes"
    );
    assert_eq!(
        &out.stdout[..4],
        &[0x28, 0xB5, 0x2F, 0xFD],
        "compressed output must start with zstd magic bytes"
    );
}

#[test]
fn subgraph_selector_no_match_exits_1() {
    let out = Command::new(omts_bin())
        .args([
            "subgraph",
            fixture("graph-query.omts").to_str().expect("path"),
            "--node-type",
            "facility",
        ])
        .output()
        .expect("run omts subgraph --node-type facility (no facilities in graph-query)");
    assert_eq!(
        out.status.code(),
        Some(1),
        "expected exit 1 when selectors match nothing"
    );
}
