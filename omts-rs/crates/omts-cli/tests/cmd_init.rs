//! Integration tests for `omts init`.
...
fn init_two_calls_produce_different_salts() {
    let out1 = Command::new(omts_bin())
        .arg("init")
        .output()
        .expect("run omts init first");
    let out2 = Command::new(omts_bin())
        .arg("init")
        .output()
        .expect("run omts init second");
...
fn init_version_is_semver() {
    ...
    let value: serde_json::Value =
        serde_json::from_str(stdout.trim()).expect("valid JSON from init");
    let version = value["omts_version"]
        .as_str()
        .expect("omts_version is string");
...
fn init_output_parseable_by_inspect() {
    ...
    let inspect_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts inspect on init output");
    assert!(
...
fn init_example_output_parseable_by_inspect() {
    ...
    let inspect_out = Command::new(omts_bin())
        .args(["inspect", tmp.path().to_str().expect("path")])
        .output()
        .expect("run omts inspect on init --example output");
    assert!(
        inspect_out.status.success(),
