...
use omts_core::{OmtsFile, cbor::encode_cbor, compression::compress_zstd};
...
/// Validates that `omts validate` accepts a JSON file (baseline).
#[test]
...
/// Validates that `omts validate` accepts a CBOR-encoded file.
#[test]
...
/// Validates that `omts validate` accepts a zstd-compressed JSON file.
#[test]
...
/// Validates that `omts validate` accepts a zstd-compressed CBOR file.
#[test]
...
/// Validates that `omts inspect` accepts a CBOR-encoded file.
#[test]
...
/// Validates that `omts inspect` accepts a zstd-compressed file.
#[test]
...
/// Validates that `omts diff` accepts files with different encodings.
#[test]
...
fn verbose_reports_encoding_json() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts inspect --verbose");
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn verbose_reports_encoding_cbor() {
    ...
    let out = Command::new(omts_bin())
        .args(["--verbose", "inspect", tmp.path().to_str().expect("path")])
        .output()
        .expect("run omts inspect --verbose cbor");
    assert_eq!(
        out.status.code(),
        Some(0),
