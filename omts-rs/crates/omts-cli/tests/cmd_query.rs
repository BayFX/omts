//! Integration tests for `omts query`.
...
fn query_no_selectors_exits_2() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query (no selectors)");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn query_nonexistent_file_exits_2() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query on nonexistent file");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn query_no_match_exits_1() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --node-type facility on graph-query.omts");
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn query_node_type_organization_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --node-type organization");
    assert!(
        out.status.success(),
...
fn query_node_type_facility_in_full_featured_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --node-type facility on full-featured.omts");
    assert!(
        out.status.success(),
...
fn query_identifier_scheme_lei_exits_0() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --identifier lei");
    assert!(
        out.status.success(),
...
fn query_identifier_scheme_value_exits_0() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --identifier lei:VALUE");
    assert!(
        out.status.success(),
...
fn query_identifier_scheme_value_no_match_exits_1() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --identifier lei:NONEXISTENT");
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn query_jurisdiction_de_exits_0() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --jurisdiction DE");
    assert!(
        out.status.success(),
...
fn query_jurisdiction_us_no_match_exits_1() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --jurisdiction US (no match)");
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn query_name_substring_exits_0() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --name Alpha");
    assert!(
        out.status.success(),
...
fn query_name_case_insensitive_exits_0() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --name alpha (lowercase)");
    assert!(
        out.status.success(),
...
fn query_name_no_match_exits_1() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --name ZZZ (no match)");
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn query_count_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --node-type organization --count");
    assert!(
        out.status.success(),
...
fn query_count_output_format_human() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --count human");
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
...
fn query_count_output_format_json() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --format json --count");
    assert!(
        out.status.success(),
...
fn query_format_json_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --format json");
    assert!(
        out.status.success(),
...
fn query_format_json_nodes_array_is_non_empty_for_organization() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --format json --node-type organization");
    let stdout = String::from_utf8_lossy(&out.stdout);
    let value: serde_json::Value =
        serde_json::from_str(stdout.trim()).expect("valid JSON from query");
...
fn query_human_output_contains_header() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query human output");
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
...
fn query_node_type_and_jurisdiction_intersection() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --node-type organization --jurisdiction DE");
    assert!(
        out.status.success(),
        "expected exit 0; exit: {:?}",
        out.status.code()
...
fn query_node_type_and_name_combined() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --node-type organization --name Beta");
    assert!(
        out.status.success(),
        "expected exit 0; exit: {:?}",
        out.status.code()
...
fn query_multiple_node_types_or_composed() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --node-type organization --node-type facility");
    assert!(
        out.status.success(),
        "expected exit 0; exit: {:?}",
        out.status.code()
...
fn query_identifier_and_jurisdiction_combined() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --identifier lei --jurisdiction DE");
    assert!(
        out.status.success(),
...
fn query_edge_type_supplies_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --edge-type supplies");
    assert!(
        out.status.success(),
...
fn query_edge_type_nonexistent_exits_1() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --edge-type ownership (not in graph-query)");
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn query_matched_counts_on_stderr() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query (check stderr)");
...
fn query_label_key_only_exits_0() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --label tier");
    assert!(
        out.status.success(),
...
fn query_label_key_value_exits_0() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --label tier=1");
    assert!(
        out.status.success(),
...
fn query_label_no_match_exits_1() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query --label region (no match)");
    assert_eq!(
        out.status.code(),
        Some(1),
