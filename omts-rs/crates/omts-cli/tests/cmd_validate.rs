//! Integration tests for `omts validate`.
...
fn validate_level_1_minimal_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate --level 1");
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn validate_level_3_minimal_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate --level 3");
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn validate_json_format_minimal_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate -f json");
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn validate_quiet_suppresses_warnings_on_stderr() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate --quiet");
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.is_empty(),
