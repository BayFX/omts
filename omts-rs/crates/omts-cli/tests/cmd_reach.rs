//! Integration tests for `omts reach`.
...
fn reach_incoming_direction_traverses_upstream() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts reach --direction incoming");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("org-c"),
...
fn reach_unknown_node_exits_1() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts reach with unknown node");
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn reach_nonexistent_file_exits_2() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts reach nonexistent");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn reach_invalid_json_exits_2() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts reach bad-json");
    assert_eq!(
        out.status.code(),
        Some(2),
