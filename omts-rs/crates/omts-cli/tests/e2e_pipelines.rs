...
//!
//! These tests invoke the `omts` binary and chain commands together via
...
/// `omts init` output piped to `omts validate` must exit 0.
#[test]
fn pipeline_init_validate() {
    let init_out = Command::new(omts_bin())
        .arg("init")
        .output()
        .expect("run omts init");
    assert!(
    ...
    let validate_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate on init output");
    assert_eq!(
...
/// `omts init --example` output must also pass validation.
#[test]
fn pipeline_init_example_validate() {
    let init_out = Command::new(omts_bin())
        .args(["init", "--example"])
        .output()
        .expect("run omts init --example");
    assert!(
        init_out.status.success(),
    ...
    let validate_out = Command::new(omts_bin())
        .args(["validate", tmp.path().to_str().expect("path")])
        .output()
        .expect("run omts validate on init --example output");
    assert_eq!(
        validate_out.status.code(),
        Some(0),
...
fn pipeline_merge_validate() {
    let merge_out = Command::new(omts_bin())
        .args([
            "merge",
    ...
    let validate_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate on merge output");
    assert_eq!(
        validate_out.status.code(),
        Some(0),
...
fn pipeline_merge_redact_validate() {
    ...
    let redact_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts redact on merged output");
    assert_eq!(
        redact_out.status.code(),
        Some(0),
    ...
    let validate_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate on merge+redact output");
    assert_eq!(
        validate_out.status.code(),
        Some(0),
...
/// `omts subgraph` output can be passed to `omts inspect`.
#[test]
fn pipeline_subgraph_inspect() {
    ...
    let inspect_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts inspect on subgraph output");
    assert_eq!(
...
/// `omts subgraph` output passes `omts validate`.
#[test]
fn pipeline_subgraph_validate() {
    let subgraph_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts subgraph --expand 1");
    assert_eq!(
        subgraph_out.status.code(),
        Some(0),
        "subgraph must succeed; stderr: {}",
...
fn pipeline_convert_diff_identical() {
    let convert_out = Command::new(omts_bin())
        .args([
            "convert",
            "--compact",
    ...
    let diff_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts diff on converted output");
    assert_eq!(
...
fn pipeline_merge_diff_shows_additions() {
    ...
    let diff_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts diff merge-a vs merged");
    assert_eq!(
        diff_out.status.code(),
        Some(1),
