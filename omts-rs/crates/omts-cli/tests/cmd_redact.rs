//! Integration tests for `omts redact`.
...
fn redact_to_partner_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts redact --scope partner");
    assert_eq!(
        out.status.code(),
        Some(0),
        "expected exit 0; stderr: {}",
...
fn redact_less_restrictive_scope_exits_1() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts redact less-restrictive scope");
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn redact_invalid_json_exits_2() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts redact bad-json");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn redact_nonexistent_file_exits_2() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts redact nonexistent");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn redact_to_cbor_starts_with_cbor_tag() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts redact --to cbor");
    assert_eq!(
        out.status.code(),
        Some(0),
        "expected exit 0; stderr: {}",
...
fn redact_to_cbor_passes_validate_and_removes_person_nodes() {
    ...
    let validate_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate on redacted CBOR output");
    assert_eq!(
        validate_out.status.code(),
        Some(0),
    ...
    let convert_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts convert cbor to json");
...
fn redact_compress_starts_with_zstd_magic() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts redact --compress");
    assert_eq!(
        out.status.code(),
        Some(0),
        "expected exit 0; stderr: {}",
...
fn redact_cbor_compress_passes_validate() {
    let redact_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts redact --to cbor --compress");
    assert_eq!(
        redact_out.status.code(),
        Some(0),
        "expected exit 0; stderr: {}",
    ...
    let validate_out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate on redacted cbor+zstd output");
    assert_eq!(
        validate_out.status.code(),
        Some(0),
