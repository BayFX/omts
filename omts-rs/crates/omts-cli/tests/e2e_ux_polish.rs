...
//! - Broken pipe: `omts inspect file | head -1` exits 0 with empty stderr
...
fn broken_pipe_inspect_head_exits_0() {
    ...
    let _ = inspect_child.wait().expect("wait omts inspect");
...
/// Piping `omts diff` output through `head -1` must not produce a broken-pipe
/// error on stderr and must exit 0 (from head's perspective).
///
/// The key behavior being tested is that omts exits cleanly when the read end
...
fn broken_pipe_diff_head_no_stderr_error() {
    let mut omts_child = Command::new(omts_bin())
        .args([
            "diff",
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn omts diff");

    let diff_stdout = omts_child.stdout.take().expect("diff stdout");
    ...
    let _ = omts_child.wait().expect("wait omts diff");
...
fn quiet_mode_suppress_summary_but_not_errors() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts --quiet validate invalid-edge.omts");
...
fn quiet_mode_no_summary_for_valid_file() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts --quiet validate minimal.omts");
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn quiet_mode_does_not_suppress_fatal_parse_error() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts --quiet validate bad-json");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn quiet_mode_does_not_suppress_file_not_found_error() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts --quiet validate nonexistent");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn verbose_validate_shows_timing_ms() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts --verbose validate minimal.omts");
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn error_file_not_found_contains_path() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate nonexistent");
...
fn error_json_parse_contains_line_column() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate bad-json");
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
/// A file-not-found error on `omts diff` must name the missing file.
#[test]
fn error_diff_file_a_not_found_contains_path() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts diff nonexistent-a");
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
/// A node-not-found error on `omts reach` must name the missing node ID.
#[test]
fn error_reach_node_not_found_contains_node_id() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts reach unknown-node");
...
/// A node-not-found error on `omts path` must include a hint about `inspect`.
#[test]
fn error_path_node_not_found_contains_hint() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts path unknown node");
    assert_eq!(out.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
fn error_redact_scope_message_contains_scope() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts redact less-restrictive");
    assert_eq!(out.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
/// `omts subgraph` with a node ID not in the graph exits 1.
...
fn subgraph_unknown_node_error_contains_node_id() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts subgraph unknown-node");
    assert_eq!(out.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
/// `omts query` with no selector flags exits 2 (invalid argument).
...
fn query_no_selectors_error_message_is_descriptive() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts query with no selectors");
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        !stderr.is_empty(),
...
fn validation_error_goes_to_stderr_not_stdout() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts validate invalid-edge.omts");
    assert!(
        out.stdout.is_empty(),
...
fn file_not_found_error_goes_to_stderr() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts inspect nonexistent");
    assert_eq!(out.status.code(), Some(2));
    assert!(
        out.stdout.is_empty(),
...
/// `omts validate` on a file with L1 errors exits 1 and error contains `[E]`.
