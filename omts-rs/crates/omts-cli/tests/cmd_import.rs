//! Integration tests for `omts import`.
...
#[test]
fn import_example_xlsx_has_omts_version_field() {
    let out = Command::new(omts_bin())
        .args([
            "import",
            excel_fixture("omts-import-example.xlsx")
    ...
    assert!(
        parsed.get("omts_version").is_some(),
        "output must contain 'omts_version' field; output: {parsed}"
    );
}
...
fn import_nonexistent_file_exits_2() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts import (missing file)");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn import_output_flag_writes_file() {
    ...
    assert!(
        parsed.get("omts_version").is_some(),
        "written file must contain 'omts_version'"
    );
}
...
fn import_default_format_is_excel() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts import (no --format)");
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn import_explicit_format_excel_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts import --format excel");
    assert_eq!(
        out.status.code(),
        Some(0),
        "expected exit 0; stderr: {}",
...
fn import_unknown_format_exits_2() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts import --input-format csv");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn import_supplier_list_example_exits_0() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts import supplier list example");
    assert_eq!(
        out.status.code(),
        Some(0),
        "expected exit 0; stderr: {}",
...
fn import_supplier_list_template_exits_0() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts import supplier list template");
