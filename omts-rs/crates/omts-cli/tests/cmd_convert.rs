//! Integration tests for `omts convert`.
...
fn convert_minimal_pretty_preserves_fields() {
    ...
    assert_eq!(value["omts_version"], "0.1.0");
...
fn convert_compact_is_smaller_than_pretty() {
    let pretty_out = Command::new(omts_bin())
        .args([
            "convert",
            "--pretty",
...
fn convert_round_trips_full_featured() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts convert full-featured");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    let stdout = String::from_utf8_lossy(&out.stdout);
    let value: serde_json::Value =
...
fn convert_preserves_unknown_fields() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts convert with unknown fields");
    assert!(out.status.success(), "exit code: {:?}", out.status.code());
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
...
fn convert_nonexistent_file_exits_2() {
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts convert nonexistent");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn convert_invalid_json_exits_2() {
    ...
    let out = Command::new(omts_bin())
        .args(["convert", tmp.path().to_str().expect("path")])
        .output()
        .expect("run omts convert bad-json");
    assert_eq!(
        out.status.code(),
        Some(2),
