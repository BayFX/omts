//! End-to-end integration tests for the `omts redact` command.
//!
...
/// Run `omts redact --scope <scope> <file>` and return the raw output.
fn run_redact(scope: &str, fixture_name: &str) -> std::process::Output {
    Command::new(omts_bin())
        .args([
            "redact",
            "--scope",
            scope,
...
/// Validate a JSON value by writing to a temp file and calling `omts validate`.
...
fn redaction_partner_to_internal_exits_1() {
    ...
    let out = Command::new(omts_bin())
        ...
        .output()
        .expect("run omts redact partner→internal");
    assert_eq!(
        out.status.code(),
        Some(1),
        "attempting less-restrictive scope must exit 1"
