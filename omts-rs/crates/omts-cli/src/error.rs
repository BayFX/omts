...
///
/// [`CliError`] is the top-level error type for the `omts` binary. Every
...
/// All error conditions that the `omts` CLI can produce.
///
...
pub enum CliError {
    ...
    /// The input is not a valid OMTS file (not valid JSON or missing required
    /// fields).
    ///
    /// This is distinct from I/O errors: the bytes were read successfully but
    /// could not be parsed as an [`omts_core::OmtsFile`].
    ParseFailed {
    ...
    /// An internal error that indicates a bug in omts.
    ///
    ...
    pub fn message(&self) -> String {
        match self {
            ...
            Self::ParseFailed { detail } => {
                format!(
                    "error: failed to parse input as an OMTS file: {detail}\n\
                     hint: validate the JSON syntax and ensure all required fields \
                     (omts_version, snapshot_date, file_salt, nodes, edges) are present"
                )
            }
            ...
            Self::NodeNotFound { node_id } => {
                format!(
                    "error: node not found: {node_id:?}\n\
                     hint: run `omts inspect <file>` to list node IDs present in the graph"
                )
            }
            ...
            Self::GraphBuildError { detail } => {
                format!(
                    "error: could not build graph from input: {detail}\n\
                     hint: run `omts validate <file>` to check for structural errors first"
                )
            }
            ...
            Self::InvalidArgument { detail } => {
                format!(
                    "error: invalid argument: {detail}\n\
                     hint: run `omts <subcommand> --help` for usage information"
                )
            }
...
///
/// Call this for error paths that indicate a bug in omts rather than a
/// user input problem.
pub fn internal_error_message(detail: &str) -> String {
    format!(
        "error: internal error: {detail}\n\
         This is a bug in omts. Please report it at \
         https://github.com/omts/omts-rs/issues"
    )
}
...
mod tests {
    ...
    fn internal_error_message_contains_bug_report_url() {
        ...
        let msg = e.message();
        assert!(
            msg.contains("https://github.com/omts/omts-rs/issues"),
    ...
    fn parse_failed_message_contains_guidance() {
        ...
        let msg = e.message();
        assert!(
            msg.contains("omts_version") || msg.contains("required fields"),
