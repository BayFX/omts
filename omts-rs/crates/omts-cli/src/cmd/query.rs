//! Implementation of `omts query <file> [selector flags]`.
//!
...
use omts_core::graph::selector_match;
...
pub fn run(
    ...
    let matched_nodes: Vec<&omts_core::Node> = result
        .node_indices
        .iter()
        .map(|&i| &file.nodes[i])
        .collect();

    let matched_edges: Vec<&omts_core::Edge> = result
        .edge_indices
        .iter()
        .map(|&i| &file.edges[i])
        .collect();
...
fn print_human<W: std::io::Write>(
    w: &mut W,
    nodes: &[&omts_core::Node],
    edges: &[&omts_core::Edge],
) -> std::io::Result<()> {
...
/// Returns a display string for a [`omts_core::NodeTypeTag`].
fn node_type_display(tag: &omts_core::NodeTypeTag) -> String {
    match tag {
        omts_core::NodeTypeTag::Known(t) => serde_json::to_value(t)
            .ok()
            .and_then(|v| v.as_str().map(str::to_owned))
            .unwrap_or_else(|| format!("{t:?}")),
        omts_core::NodeTypeTag::Extension(s) => s.clone(),
    }
}

/// Returns a display string for a [`omts_core::EdgeTypeTag`].
fn edge_type_display(tag: &omts_core::EdgeTypeTag) -> String {
    match tag {
        omts_core::EdgeTypeTag::Known(t) => serde_json::to_value(t)
            .ok()
            .and_then(|v| v.as_str().map(str::to_owned))
            .unwrap_or_else(|| format!("{t:?}")),
        omts_core::EdgeTypeTag::Extension(s) => s.clone(),
    }
}
...
mod tests {
    ...
    fn test_human_output_contains_header() {
        ...
        let mut buf = Vec::new();
        let nodes: Vec<&omts_core::Node> = file.nodes.iter().collect();
        let edges: Vec<&omts_core::Edge> = vec![];
    ...
    fn test_human_output_contains_edge_data() {
        let file: OmtsFile = serde_json::from_str(SAMPLE_FILE).expect("parse");
        let mut buf = Vec::new();
        let nodes: Vec<&omts_core::Node> = vec![];
        let edges: Vec<&omts_core::Edge> = file.edges.iter().collect();
        print_human(&mut buf, &nodes, &edges).expect("write");
        let output = String::from_utf8(buf).expect("utf8");
