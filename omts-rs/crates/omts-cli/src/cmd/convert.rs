//! Implementation of `omts convert <file>`.
//!
...
/// delegated to [`omts_core::convert`] so that CBOR encoding and zstd
...
fn serialize(
    ...
    match to {
        TargetEncoding::Cbor => {
            ...
            omts_core::convert(file, omts_core::Encoding::Cbor, compress)
        ...
        TargetEncoding::Json => {
            ...
            if use_compact {
                omts_core::convert(file, omts_core::Encoding::Json, compress)
                    .map_err(|e| convert_error_to_cli(&e))
            } else {
...
/// `omts_core::convert` always produces compact JSON, so pretty-printing
...
/// Maps a [`omts_core::ConvertError`] to a [`CliError`].
fn convert_error_to_cli(e: &omts_core::ConvertError) -> CliError {
...
mod tests {
    ...
    use omts_core::{OmtsFile, decode_cbor, decompress_zstd};
