//! Implementation of `omts subgraph <file> [node-id...] [selector flags]`.
//!
...
use omts_core::graph::queries::Direction as CoreDirection;
use omts_core::graph::{QueryError, build_graph, ego_graph, induced_subgraph, selector_subgraph};
use omts_core::newtypes::CalendarDate;
...
fn serialize(file: &OmtsFile, to: &TargetEncoding, compress: bool) -> Result<Vec<u8>, CliError> {
    match to {
        TargetEncoding::Cbor => omts_core::convert(file, omts_core::Encoding::Cbor, compress)
        ...
        TargetEncoding::Json => {
            ...
            if compress {
                omts_core::compress_zstd(&json_bytes).map_err(|e| CliError::InternalError {
...
fn compute_expanded_subgraph(
    graph: &omts_core::graph::OmtsGraph,
    file: &OmtsFile,
...
mod tests {
    ...
    fn test_selector_produces_valid_omts() {
        ...
        let graph = omts_core::build_graph(&file).expect("build");
        ...
        let subgraph = omts_core::graph::selector_subgraph(&graph, &file, &selector_set, 0)
    ...
    fn test_selector_expand_1_includes_adjacent() {
        ...
        let subgraph = omts_core::graph::selector_subgraph(&graph, &file, &selector_set, 1)
            .expect("extract subgraph");
