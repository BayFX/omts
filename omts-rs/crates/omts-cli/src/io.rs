...
///
/// This module is the single entry point for all input I/O in the `omts`
/// binary. `omts-core` never touches the filesystem; all reading happens here.
///
/// Key behaviours:
...
/// - Multi-encoding parse via `omts_core::parse_omts` (JSON, CBOR, zstd).
...
use omts_core::{Encoding, OmtsDecodeError, OmtsFile, parse_omts};
...
/// Reads raw bytes from `source` and parses them as an OMTS file.
///
...
fn decode_error_to_cli(e: OmtsDecodeError, source: &str) -> CliError {
    match e {
        ...
        OmtsDecodeError::Compression(inner) => {
            use omts_core::CompressionError;
            match inner {
