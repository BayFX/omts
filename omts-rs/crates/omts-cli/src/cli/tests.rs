...
/// `omts validate --help` must mention `--level` and `FILE`.
#[test]
...
/// `omts merge --help` must mention `--strategy`.
#[test]
fn test_merge_help() {
    let mut cmd = Cli::command();
    let sub = cmd
...
/// `omts redact --help` must mention `--scope`.
#[test]
fn test_redact_help() {
    let mut cmd = Cli::command();
    let sub = cmd
...
/// `omts inspect --help` must mention `FILE`.
#[test]
...
/// `omts diff --help` must mention `--ids-only`.
#[test]
fn test_diff_help() {
    let mut cmd = Cli::command();
    let sub = cmd
...
/// `omts convert --help` must mention `--compact`, `--pretty`, `--to`, and `--compress`.
#[test]
...
/// `omts convert --to json` parses correctly and defaults to pretty output.
#[test]
fn test_convert_to_json_flag() {
    let cli = Cli::try_parse_from(["omts", "convert", "--to", "json", "supply-chain.omts"])
...
/// `omts convert --to cbor` parses correctly.
#[test]
fn test_convert_to_cbor_flag() {
    let cli = Cli::try_parse_from(["omts", "convert", "--to", "cbor", "supply-chain.omts"])
...
/// `omts convert --compress` parses correctly.
#[test]
fn test_convert_compress_flag() {
    let cli = Cli::try_parse_from(["omts", "convert", "--compress", "supply-chain.omts"])
...
/// `omts convert --to cbor --compress` is accepted.
#[test]
...
/// `omts reach --help` must mention `--depth`, `--direction`, and `NODE_ID`.
#[test]
fn test_reach_help() {
    let mut cmd = Cli::command();
    let sub = cmd
...
/// `omts path --help` must mention `--max-paths` and `--max-depth`.
#[test]
fn test_path_help() {
    let mut cmd = Cli::command();
    let sub = cmd
...
/// `omts subgraph --help` must mention `--expand` and selector flags.
#[test]
...
/// `omts init --help` must mention `--example`.
#[test]
fn test_init_help() {
    let mut cmd = Cli::command();
    let sub = cmd
...
fn test_path_or_stdin_parses_dash_as_stdin() {
    let cli = Cli::try_parse_from(["omts", "validate", "-"]).expect("should parse validate -");
    match cli.command {
...
fn test_path_or_stdin_parses_real_path() {
    let cli = Cli::try_parse_from(["omts", "validate", "supply-chain.omts"])
...
fn test_quiet_verbose_conflict() {
    let result = Cli::try_parse_from(["omts", "--quiet", "--verbose", "validate", "-"]);
    assert!(
        result.is_err(),
...
fn test_max_file_size_default() {
    let cli = Cli::try_parse_from(["omts", "validate", "-"])
...
fn test_max_file_size_cli_override() {
    let cli = Cli::try_parse_from(["omts", "--max-file-size", "1048576", "validate", "-"])
...
fn test_format_flag_json() {
    let cli = Cli::try_parse_from(["omts", "--format", "json", "validate", "-"])
...
fn test_format_flag_default_is_human() {
    let cli =
        Cli::try_parse_from(["omts", "validate", "-"]).expect("should parse without --format");
    assert!(
...
fn test_merge_requires_two_files() {
    let result = Cli::try_parse_from(["omts", "merge", "only-one.omts"]);
    assert!(
        result.is_err(),
...
fn test_merge_accepts_two_files() {
    let cli = Cli::try_parse_from(["omts", "merge", "a.omts", "b.omts"])
...
fn test_subgraph_no_ids_no_selectors_parses() {
    let cli = Cli::try_parse_from(["omts", "subgraph", "graph.omts"])
...
fn test_validate_level_range_valid() {
    for level in ["1", "2", "3"] {
        let cli = Cli::try_parse_from(["omts", "validate", "--level", level, "supply-chain.omts"])
...
fn test_validate_level_range_out_of_bounds() {
    let result = Cli::try_parse_from(["omts", "validate", "--level", "0", "supply-chain.omts"]);
    assert!(result.is_err(), "--level 0 should be rejected");

    let result = Cli::try_parse_from(["omts", "validate", "--level", "4", "supply-chain.omts"]);
...
fn test_redact_scope_variants() {
    for scope in ["public", "partner", "internal"] {
        let cli = Cli::try_parse_from(["omts", "redact", "--scope", scope, "supply-chain.omts"])
...
fn test_convert_compact_conflicts_with_pretty() {
    let result = Cli::try_parse_from(["omts", "convert", "--compact", "--pretty", "file.omts"]);
    assert!(
        result.is_err(),
...
/// `omts query --help` must mention `--node-type`, `--edge-type`, and `--count`.
#[test]
fn test_query_help() {
    let mut cmd = Cli::command();
    let sub = cmd
...
/// `omts query` with `--count` flag parses correctly.
#[test]
...
/// `omts query` with multiple `--node-type` flags parses all values.
#[test]
...
/// `omts query` with `--label key=value` parses correctly.
#[test]
fn test_query_label_flag_parses() {
    let cli = Cli::try_parse_from(["omts", "query", "--label", "tier=1", "graph.omts"])
...
/// `omts query` with `--identifier scheme:value` parses correctly.
#[test]
...
/// `omts query` with `--jurisdiction CC` parses correctly.
#[test]
fn test_query_jurisdiction_flag_parses() {
    let cli = Cli::try_parse_from(["omts", "query", "--jurisdiction", "DE", "graph.omts"])
...
/// `omts query` with `--name pattern` parses correctly.
#[test]
fn test_query_name_flag_parses() {
    let cli = Cli::try_parse_from(["omts", "query", "--name", "acme", "graph.omts"])
