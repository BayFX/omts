...
use omts_core::enums::{DisclosureScope, EdgeType, EdgeTypeTag};
use omts_core::graph::queries::{self, Direction};
use omts_core::validation::{ValidationConfig, validate};
use omts_core::{build_graph, diff, merge, redact};

fn medium_file() -> omts_core::OmtsFile {
...
fn induced_subgraph_invariants() {
    ...
    let sub = omts_core::graph::extraction::induced_subgraph(&graph, &file, &node_ids)
...
fn ego_graph_returns_valid_subgraph() {
    ...
    let ego = omts_core::graph::extraction::ego_graph(&graph, &file, &center, 2, Direction::Both)
...
fn redact_to_partner_scope() {
    let file = generate_supply_chain(&SizeTier::Small.config(42));
    let retain: HashSet<omts_core::NodeId> = file.nodes.iter().map(|n| n.id.clone()).collect();
