...
fn assert_l1_valid(file: &omts_core::OmtsFile, label: &str) {
...
fn generated_small_round_trips_through_json() {
    ...
    let back: omts_core::OmtsFile = serde_json::from_str(&json).expect("deserialize");
...
fn graph_builds_from_generated_file() {
    ...
    let graph = omts_core::build_graph(&file).expect("graph should build from generated file");
...
fn cyclic_variant_generates_cycles() {
    ...
    let graph = omts_core::build_graph(&file).expect("builds");
    ...
    let cycles = omts_core::graph::cycles::detect_cycles(&graph, &all_types);
...
mod proptest_tests {
    ...
    proptest! {
        ...
        fn graph_build_succeeds_on_all_generated_files(seed in 0u64..1000) {
            let file = generate_supply_chain(&SizeTier::Small.config(seed));
            let graph = omts_core::build_graph(&file).expect("graph build must succeed");
