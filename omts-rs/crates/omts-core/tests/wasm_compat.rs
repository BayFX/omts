//! WASM compatibility documentation tests for `omts-core`.
//!
//! These tests run on native targets and verify that the core types used by
//! `omts-core` can be constructed and exercised without any platform-specific
...
//!
//! `omts-core` must compile for `wasm32-unknown-unknown`.  The CI pipeline
//! enforces this via `just wasm-check`:
//!
//! ```text
//! cargo build -p omts-core --target wasm32-unknown-unknown
//! ```
//!
//! ## Forbidden imports
//!
//! Production code under `crates/omts-core/src/` must not import:
//! - `std::fs`
//! - `std::net`
//! - `std::process`
//!
//! All I/O belongs in `omts-cli`.
...
fn node_id(s: &str) -> omts_core::NodeId {
    omts_core::NodeId::try_from(s).expect("valid NodeId")
}

fn edge_id(s: &str) -> omts_core::EdgeId {
    omts_core::EdgeId::try_from(s).expect("valid EdgeId")
}
...
///
/// This is the same code path exercised when `omts-core` is loaded inside a
/// WASM module.
#[test]
...
/// [`omts_core::validate`] accepts and validates an [`OmtsFile`] using only
...
/// [`omts_core::redact`] produces a redacted file using only in-memory
...
fn wasm_compat_redaction_no_io() {
    ...
    let mut retain: HashSet<omts_core::NodeId> = HashSet::new();
    retain.insert(node_id("org-pub"));

    let output =
        omts_core::redact(&file, DisclosureScope::Partner, &retain).expect("redact must succeed");

    assert_eq!(output.disclosure_scope, Some(DisclosureScope::Partner));
    assert_eq!(output.nodes.len(), 2);
}

/// The [`omts_core::boundary_ref_value`] function uses `sha2` under the hood.
/// `sha2` compiles for WASM32 and requires no platform entropy — it is a pure
/// hash function.
#[test]
fn wasm_compat_sha2_no_io() {
    use omts_core::boundary_ref_value;
    use omts_core::decode_salt;

    let file_salt = salt();
    let salt_bytes = decode_salt(&file_salt).expect("decode_salt must succeed");

    let id = omts_core::types::Identifier {
        scheme: "lei".to_owned(),
    ...
    let canonical_id = omts_core::CanonicalId::from_identifier(&id);
