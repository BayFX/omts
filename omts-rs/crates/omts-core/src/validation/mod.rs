/// Diagnostic types and rule dispatch for the OMTS validation engine.
///
...
/// `omts-rs/docs/validation.md` Section 2.
///
...
///
/// Maps directly to the three validation levels defined in the OMTS spec:
...
///
/// Each variant corresponds to exactly one rule defined in the OMTS
...
impl RuleId {
    ...
    /// OMTS specification (e.g. `"L1-GDM-03"`, `"L2-EID-04"`).
...
/// A single validation finding produced by the OMTS validation engine.
///
...
/// The collected output of a validation pass on a parsed OMTS graph.
///
...
/// The top-level result type returned by the OMTS validator.
///
...
pub enum ValidateOutput {
    /// The input file could not be parsed as a valid OMTS file.
    ///
...
///
/// Maps to the three tiers defined in the OMTS specification:
...
///
/// Each rule in the OMTS validation engine implements this trait.  Rules push
...
///
/// ```
/// # use omts_core::ValidationConfig;
...
/// whose level is enabled in `config`.  Rules are compiled into `omts-core`;
