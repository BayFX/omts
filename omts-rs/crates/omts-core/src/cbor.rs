...
mod tests {
    ...
    fn minimal_file() -> OmtsFile {
        let json = format!(
            r#"{{"omts_version":"1.0.0","snapshot_date":"2026-02-19","file_salt":"{SALT}","nodes":[],"edges":[]}}"#
        );
    ...
    /// `omts_version` key is present in the decoded file (SPEC-007 Section 4.6).
    #[test]
    fn decoded_file_has_omts_version() {
        let file = minimal_file();
        let cbor = encode_cbor(&file).expect("encode");
        let decoded = decode_cbor(&cbor).expect("decode");
        assert_eq!(decoded.omts_version, file.omts_version);
    }
