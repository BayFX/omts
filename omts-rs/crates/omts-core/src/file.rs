/// Top-level OMTS file representation.
///
/// [`OmtsFile`] is the root type for a serialised/deserialised OMTS snapshot.
...
/// (data-model.md Section 8.5) requires `omts_version` to be the first key in
...
/// data loss through an older `omts-core`.  **Never** add
...
/// The top-level OMTS file representation.
///
...
/// # Required fields
///
/// - `omts_version` — specification version this file conforms to
...
pub struct OmtsFile {
    /// OMTS specification version this file conforms to (e.g. `"1.0.0"`).
    ///
...
mod tests {
    ...
    fn omts_file_minimal_parse() {
        ...
        assert_eq!(f.omts_version, semver("1.0.0"));
    ...
    fn omts_file_full_optional_fields() {
        ...
        let f: OmtsFile = serde_json::from_str(&json).expect("deserialize");
        assert_eq!(f.omts_version, semver("1.2.0"));
    ...
    /// The first JSON key in the serialised output must be `omts_version`.
    ///
    ...
    #[test]
    fn omts_file_omts_version_is_first_key() {
        let f = minimal_file();
        let json = serde_json::to_string(&f).expect("serialize");

        let first_key_pos = json.find("omts_version").expect("omts_version in output");
        for other_key in &[
            "snapshot_date",
            "file_salt",
            "disclosure_scope",
            ...
            if let Some(pos) = json.find(other_key) {
                assert!(
                    pos > first_key_pos,
                    "`omts_version` must precede `{other_key}` in serialised output"
                );
            }
        }
    }
    ...
    fn omts_file_field_order_required_fields() {
        ...
        let pos_version = json.find("omts_version").expect("omts_version");
        ...
        assert!(
            ...
            "omts_version must precede snapshot_date"
        );
    ...
    /// Missing `omts_version` must fail deserialization.
    #[test]
    fn omts_file_missing_omts_version_fails() {
        let json = format!(
            r#"{{
                "snapshot_date": "2026-02-19",
                "file_salt": "{SALT}",
        ...
        assert!(result.is_err(), "missing omts_version should fail");
    }
    ...
    /// An invalid `omts_version` format fails deserialization.
    #[test]
    fn omts_file_invalid_omts_version_fails() {
        let json = format!(
            r#"{{
                "omts_version": "not-semver",
                "snapshot_date": "2026-02-19",
                "file_salt": "{SALT}",
                "nodes": [],
                "edges": []
        ...
        let result: Result<OmtsFile, _> = serde_json::from_str(&json);
        assert!(result.is_err(), "invalid omts_version format should fail");
    }
