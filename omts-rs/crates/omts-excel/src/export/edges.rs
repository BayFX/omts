...
use omts_core::structures::{Edge, Node};
...
fn conflicts_str(extra: &omts_core::dynvalue::DynMap) -> String {
    extra
        .get("_conflicts")
...
pub fn labels_to_str(labels: &[omts_core::types::Label]) -> String {
    labels
        .iter()
        .map(|l| match &l.value {
...
pub fn build_attestation_row_map(nodes: &[Node]) -> HashMap<String, u32> {
    ...
    for node in nodes {
        if matches!(
            &node.node_type,
            NodeTypeTag::Known(omts_core::enums::NodeType::Attestation)
        ) {
...
fn consolidation_basis_str(s: &omts_core::enums::ConsolidationBasis) -> &'static str {
    match s {
        omts_core::enums::ConsolidationBasis::Ifrs10 => "ifrs10",
        omts_core::enums::ConsolidationBasis::UsGaapAsc810 => "us_gaap_asc810",
        omts_core::enums::ConsolidationBasis::Other => "other",
        omts_core::enums::ConsolidationBasis::Unknown => "unknown",
    }
}

fn event_type_str(s: &omts_core::enums::EventType) -> &'static str {
    match s {
        omts_core::enums::EventType::Merger => "merger",
        omts_core::enums::EventType::Acquisition => "acquisition",
        omts_core::enums::EventType::Rename => "rename",
        omts_core::enums::EventType::Demerger => "demerger",
        omts_core::enums::EventType::SpinOff => "spin_off",
    }
}
