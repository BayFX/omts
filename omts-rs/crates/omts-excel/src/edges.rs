...
use omts_core::dynvalue::DynValue;
use omts_core::enums::{ConsolidationBasis, EdgeType, EdgeTypeTag, EventType};
use omts_core::newtypes::{CalendarDate, EdgeId, NodeId};
use omts_core::structures::{Edge, EdgeProperties};
...
fn parse_same_as(
    ...
    for (row_idx, row) in sheet.rows().skip(1).enumerate() {
        ...
        let mut extra: BTreeMap<String, omts_core::dynvalue::DynValue> = BTreeMap::new();
        if let Some(b) = &basis {
            extra.insert(
                "basis".to_owned(),
                omts_core::dynvalue::DynValue::from(serde_json::json!(b)),
            );
        }
        if let Some(conf_str) = read_optional_string(row, &headers, "confidence") {
            extra.insert(
                "confidence".to_owned(),
                omts_core::dynvalue::DynValue::from(serde_json::json!(conf_str)),
            );
        }
