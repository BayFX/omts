...
use omts_core::newtypes::{CalendarDate, CountryCode, NodeId};
use omts_core::structures::Node;
use omts_core::types::{DataQuality, Identifier};
...
fn build_geo_value(
    ...
    sheet_name: &str,
    row_idx: usize,
) -> Result<Option<omts_core::dynvalue::DynValue>, ImportError> {
    ...
    match (lat, lon) {
        (Some(lat), Some(lon)) => Ok(Some(omts_core::dynvalue::DynValue::from(
