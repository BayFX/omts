...
use omts_core::newtypes::{CalendarDate, CountryCode, EdgeId, NodeId, SemVer};
use omts_core::structures::{Edge, EdgeProperties, Node};
...
pub fn import_supplier_list<R: Read + Seek>(
    ...
    let config = omts_core::validation::ValidationConfig {
        run_l1: true,
        run_l2: false,
        run_l3: false,
    };
    let result = omts_core::validation::validate(&omts_file, &config, None);
